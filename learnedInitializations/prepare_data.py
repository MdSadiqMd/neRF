...
def prepare_dataset(images, poses, focal):
    ...
    for i in range(N):
        ...
        all_rgbs.append(images[i].reshape(-1, 3))

    dataset = np.concatenate(
        [
            np.concatenate(all_rays_o, axis=0),
            np.concatenate(all_rays_d, axis=0),
            np.concatenate(all_rgbs, axis=0),
        ],
        axis=1,
    ).astype(np.float32)
    return dataset
...
def main():
    ...
    print(f"Training images: {len(train_indices)}, Testing: {len(test_indices)}")
